...
createProblemDataObject = function(problem){
    ...
    class CanvasAdBox {
        constructor(x, y, width, height, title, imgSrc, text) {
            this.x = x;
            this.y = y;
            this.width = width;
            this.height = height;
        ...
        updateHover(mx, my) {
            ...
            this.hoveringClose =
            this.visible &&
            mx >= closeX && mx <= closeX + 30 &&
            my >= closeY && my <= closeY + 30;
        }
        ...
        draw(ctx) {
            ...
            ctx.fillStyle = this.hoveringClose ? "#e81123" : "#0078D7";
            ...
            // Ad image
            ctx.drawImage(this.img, this.x, this.y + 30, this.width, this.height-30);
            ...
            // this.wrapText(ctx, this.text, this.width/6, this.y + 100, this.width - 110, 48);
        }
        ...
        handleClick(mx, my) {
            if (this.hoveringClose) {
                ...
                adBoxes.push(
                    new CanvasAdBox(
                        Math.random()*(window.innerWidth-700), 
                        Math.random()*(window.innerHeight-550), 
                        700, 550, 
                        "Hot Singles", 
                        "https://i.pinimg.com/originals/ca/d9/43/cad943a4d9da09cfa9d15d725e1c95f3.gif", 
                        "Hot singles are looking for you just 3 miles away! Click here now!"
                    )
                )
            }
        }
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        adBoxes.forEach(ad => ad.updateHover(mouseX, mouseY));
    });
    
    document.addEventListener("click", (e) => {
        adBoxes.forEach(ad => ad.handleClick(mouseX, mouseY));
    });
    
    function draw() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        adBoxes.forEach(ad => ad.draw(ctx));
    ...
    function hash(str) {
        ...
        return hash >>> 0; // Ensure it's a non-negative 32-bit unsigned int
    }
